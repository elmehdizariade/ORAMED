const window = { location: { search: '' } }; const document = { addEventListener: () => {}, getElementById: () => ({ classList: { toggle: () => {} }, setAttribute: () => {}, innerHTML: '' }), querySelectorAll: () => [], body: { classList: { toggle: () => {} } } }; const localStorage = { getItem: () => null, setItem: () => {} }; const alert = console.log; const confirm = () => true;
const STORAGE_KEY = 'oramed_depot_stock_v45_logo';
const ALL_TABS = [
  ['reception','Bon de rÃ©ception'],
...
const directionTabs = [
  ['mouvements','Mouvements par rÃ©fÃ©rence'],
  ['referentiel','Base']
];
...
function saveSyncConfig(config){
  ...
  updateSyncStatus(syncState.config.enabled ? 'ready' : 'local', syncState.config.enabled ? 'Base centrale configurÃ©e.' : 'Mode local seulement.');
}
...
async function pushRemoteState(reason='save'){
  ...
  updateSyncStatus('syncing', reason === 'setup' ? 'Connexion Ã  la base centrale...' : 'Envoi des donnÃ©es vers la base centrale...');
  ...
  if(!res.ok) throw new Error('Ã‰criture distante impossible.');
  const rows = await res.json();
  const row = Array.isArray(rows) && rows[0] ? rows[0] : null;
  if(row?.updated_at) syncState.lastRemoteUpdatedAt = row.updated_at;
  updateSyncStatus('online', 'Base centrale synchronisÃ©e.');
  return true;
}
async function pullRemoteState(force=false){
  ...
  try{
    ...
    syncRoleBadge();
    renderActiveTab();
    updateSyncStatus('online', 'DonnÃ©es mises Ã  jour depuis la base centrale.');
    return true;
  } finally {
...
async function initRemoteSync(){
  ...
  try{
    ...
    syncState.pollTimer = setInterval(() => {
      pullRemoteState(false).catch(err => updateSyncStatus('local', err.message || 'La synchro distante a Ã©tÃ© interrompue.'));
    }, REMOTE_POLL_INTERVAL);
  }catch(err){
    updateSyncStatus('local', err.message || 'Connexion Ã  la base centrale impossible.');
  }
}
...
function renderSyncSettings(targetId='syncSettingsHost'){
  ...
  host.innerHTML = `
    <div class="card">
      <div class="toolbar">
        <div>
          <h3 class="section-title" style="margin:0">Base centralisÃ©e</h3>
          <div class="subtitle" style="margin:6px 0 0 0">Active la mÃªme base pour le poste opÃ©rateur et le poste direction, sans toucher Ã  la logique des bons.</div>
        </div>
      ...
      <div class="sync-grid">
        ...
        <div>
          ...
          <input id="sync_key" value="${esc(cfg.anonKey)}" placeholder="ClÃ© publique Supabase">
        </div>
      </div>
      ...
      <div class="notice" style="margin-top:12px">Table attendue : <strong>${REMOTE_TABLE}</strong> avec les colonnes <strong>app_id</strong> (texte, clÃ© unique), <strong>payload</strong> (jsonb), <strong>updated_at</strong> (timestamptz, dÃ©faut now()), <strong>updated_by</strong> (texte).</div>
    </div>`;
  updateSyncStatus(syncState.status, syncState.lastMessage || (cfg.enabled ? 'Base centrale configurÃ©e.' : 'Mode local seulement.'));
}
...
function getRoleLabel(){ return isDirectionRole() ? 'Direction' : 'OpÃ©rateur'; }
...
function renderActiveTab(){
  ...
  try {
    ...
    document.getElementById('view').innerHTML = `<div class="card"><h2 class="section-title">Erreur d'affichage</h2><div class="muted" style="margin-top:8px">Un ancien essai a bloquÃ© l'onglet. Cette version a Ã©tÃ© isolÃ©e avec une nouvelle mÃ©moire locale. Recharge la page puis rÃ©essaie.</div></div>`;
  }
}
    ...
    <div class="card">
      <div class="toolbar">
        <div>
          ...
          <div class="subtitle" style="margin:6px 0 0 0">Espace de pilotage avec mouvements par rÃ©fÃ©rence et base.</div>
          <div class="role-note">Cette vue reste rÃ©servÃ©e au profil direction. La logique des bons, de l'Ã©tat de stock et de la synthÃ¨se n'a pas Ã©tÃ© touchÃ©e.</div>
        </div>
...
function computeStockRows(){
  ...
  return refs.map(ref => {
    const inLines = state.receptions.filter(doc => doc.status !== 'annulÃ©').flatMap(doc => (doc.lines||[]).filter(l => l.referenceId === ref.id).map(l => ({...l, date:doc.date, number:doc.number, fournisseur:doc.fournisseur, transporteur:doc.transporteur, chauffeur:doc.chauffeur, matricule:doc.matricule})));
    const outLines = state.sorties.filter(doc => doc.status !== 'annulÃ©').flatMap(doc => (doc.lines||[]).filter(l => l.referenceId === ref.id).map(l => ({...l, date:doc.date, number:doc.number, clientNom:doc.clientNom, clientType:doc.clientType, clientCode:doc.clientCode})));
...
function renderRefSuggestions(mode){
  ...
  if(mode === 'rec'){
    ...
    if(!refs.length){
      box.innerHTML = `<div class="autocomplete-item muted">Aucune rÃ©fÃ©rence trouvÃ©e pour ${esc(selectedSupplier)}.</div>`;
      box.classList.remove('hidden');
      hidden.value = '';
      return;
    }
    box.innerHTML = refs.map(r => `<div class="autocomplete-item ${String(r.choix||'').toLowerCase().includes('2eme') ? 'second-choice' : ''}" onclick="selectReference('${mode}','${r.id}')"><strong>${esc(r.reference)}</strong><div class="small muted">${esc(r.format)} Â· ${esc(getSupplierBrand(r.marque))} Â· ${esc(r.choix || '')}</div></div>`).join('');
    box.classList.remove('hidden');
    return;
  }
  const refs = state.references
...
function computeMovements(referenceId){
  const rows = [];
  state.receptions.filter(doc => doc.status !== 'annulÃ©').forEach(doc => (doc.lines||[]).filter(l => l.referenceId===referenceId).forEach(l => rows.push({date:doc.date,type:'ENTRÃ‰E',number:doc.number,tiers:doc.fournisseur||doc.provenance||'',metrage:Number(l.metrage||0),boxes:Number(l.boxes||0)})));
  state.sorties.filter(doc => doc.status !== 'annulÃ©').forEach(doc => (doc.lines||[]).filter(l => l.referenceId===referenceId).forEach(l => rows.push({date:doc.date,type:'SORTIE',number:doc.number,tiers:doc.clientNom||'',metrage:-Number(l.metrage||0),boxes:Number(l.boxes||0)})));
...
function cancelEditReference(){
  ...
  refreshReferencePackagingForm();
  const btn = document.getElementById('ref_submit_btn');
  if(btn) btn.textContent = 'Ajouter la rÃ©fÃ©rence';
}
...
function renderReferentiel(mountId='view'){
  ...
  const arrow = (open) => `<span class="arrow">â–¾</span>`;
  target.innerHTML = `
    <div class="card">
      <div class="toolbar" style="align-items:end">
        <div>
          <h2 class="section-title" style="margin:0">Base</h2>
          <div class="subtitle" style="margin:6px 0 0 0">RÃ©fÃ©rences dÃ©pÃ´t et clients Ã  compte utilisÃ©s dans les rÃ©ceptions, sorties et Ã©tats de stock.</div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="collapse-head ${acc.baseRefForm===false?'closed':''}" onclick="toggleAccordion('baseRefForm')">
        <h2 class="section-title" style="margin:0">Base des rÃ©fÃ©rences</h2>
        ${arrow(acc.baseRefForm!==false)}
      </div>
      ${acc.baseRefForm===false ? '' : `
        <div class="subtitle">Tu peux saisir ici le stock initial et les donnÃ©es palette pour chaque rÃ©fÃ©rence.</div>
        <input type="hidden" id="ref_edit_id">
        <div class="row">
          <div><label>RÃ©fÃ©rence</label><input id="ref_reference"></div>
        ...
        <div class="row" style="margin-top:12px">
          <div><label>mÂ² / caisse</label><input id="ref_m2" type="number" step="0.01" oninput="refreshReferencePackagingForm()"></div>
          <div><label>Stock initial (mÂ²)</label><input id="ref_initial" type="number" step="0.01"></div>
          ...
          <div><label>mÂ² / palette</label><input id="ref_m2_palette" readonly></div>
        </div>
        <div class="footer-actions"><button id="ref_submit_btn" class="success" onclick="addReference()">Ajouter la rÃ©fÃ©rence</button><button class="secondary" onclick="cancelEditReference()">Nouveau</button></div>
      `}
    </div>
    ...
    <div class="card">
      <div class="collapse-head ${acc.baseRefList===false?'closed':''}" onclick="toggleAccordion('baseRefList')">
        <div class="toolbar" style="margin:0;width:100%">
          <h2 class="section-title" style="margin:0">Liste des rÃ©fÃ©rences</h2>
          <span class="pill">${refs.length} rÃ©fÃ©rence(s)</span>
        </div>
        ${arrow(acc.baseRefList!==false)}
      </div>
      ${acc.baseRefList===false ? '' : `
        <div style="margin-top:12px;max-width:420px">
          <label>Recherche rÃ©fÃ©rence / format / fournisseur</label>
        ...
        <div class="table-wrap" style="margin-top:12px">
          <table>
            <thead><tr><th>RÃ©fÃ©rence</th><th>Format</th><th>Fournisseur</th><th>Choix</th><th>mÂ²/caisse</th><th>Stock initial</th><th>Caisse/palette</th><th></th></tr></thead>
            <tbody>
              ${refs.map(r => `<tr>
              ...
              </tr>`).join('') || `<tr><td colspan="8" class="muted">Aucune rÃ©fÃ©rence.</td></tr>`}
            </tbody>
          </table>
        </div>
      `}
    ...
    <div class="card">
      <div class="collapse-head ${acc.baseClientForm===false?'closed':''}" onclick="toggleAccordion('baseClientForm')">
        <h2 class="section-title" style="margin:0">Base des clients Ã  compte</h2>
        ${arrow(acc.baseClientForm!==false)}
      </div>
      ${acc.baseClientForm===false ? '' : `
        <div class="subtitle">Le bon de sortie pourra Ãªtre saisi en client divers ou client Ã  compte via ce code client.</div>
        ...
        <div class="row-2" style="margin-top:12px">
          <div><label>TÃ©lÃ©phone</label><input id="client_phone"></div>
    ...
    <div class="card">
      <div class="collapse-head ${acc.baseClientList===false?'closed':''}" onclick="toggleAccordion('baseClientList')">
        <div class="toolbar" style="margin:0;width:100%">
          <h2 class="section-title" style="margin:0">Liste des clients Ã  compte</h2>
      ...
      ${acc.baseClientList===false ? '' : `
        <div style="margin-top:12px;max-width:420px">
          <label>Recherche code / nom / tÃ©lÃ©phone / adresse</label>
        ...
        <div class="table-wrap" style="margin-top:12px">
          <table>
            <thead><tr><th>Code</th><th>Nom</th><th>TÃ©lÃ©phone</th><th>Adresse</th><th></th></tr></thead>
            <tbody>
              ${clients.map(c => `<tr>
              ...
              </tr>`).join('') || `<tr><td colspan="5" class="muted">Aucun client Ã  compte.</td></tr>`}
            </tbody>
          </table>
        </div>
      `}
...
function addReference(){
  ...
  if(!reference){ alert('Remplis au minimum la rÃ©fÃ©rence.'); return; }
  const exists = state.references.some(r => String(r.reference||'').toLowerCase() === reference.toLowerCase() && r.id !== editId);
  if(exists){ alert('Cette rÃ©fÃ©rence existe dÃ©jÃ .'); return; }
...
function removeReference(id){
  ...
  if(used){ alert('Impossible : cette rÃ©fÃ©rence a dÃ©jÃ  des mouvements.'); return; }
...
function addClient(){
  ...
  if(state.clients.some(c => c.code.toLowerCase() === code.toLowerCase() && c.id !== editId)){ alert('Ce code client existe dÃ©jÃ .'); return; }
  if(editId){
...
function removeClient(id){
  ...
  if(used){ alert('Impossible : ce client a dÃ©jÃ  des bons de sortie.'); return; }
...
function renderReception(){
  ...
  document.getElementById('view').innerHTML = `
    <div class="card">
      <div class="toolbar"><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><h2 class="section-title" style="margin:0">Bon de rÃ©ception</h2>${d.editingArchiveId ? '<span class="tag warn-tag">Modification archive</span>' : ''}</div><button class="secondary" onclick="newReceptionDraft()">Nouveau bon</button></div>
      <div class="row">
        <div><label>NÂ° entrÃ©e</label><input value="${esc(d.number)}" readonly></div>
    ...
    <div class="card entry-card">
      <div class="toolbar"><h2 class="section-title" style="margin:0">Ajouter une ligne du bon d'entrÃ©e</h2><span class="pill">${d.lines.length} ligne(s)</span></div>
      <div class="row">
        <div>
          <label>RÃ©fÃ©rence</label>
          <div class="autocomplete">
            <input id="rec_ref_search" placeholder="Tape le dÃ©but de rÃ©fÃ©rence" oninput="renderRefSuggestions('rec')" onfocus="renderRefSuggestions('rec')" autocomplete="off">
        ...
        <div><label>Total mÂ²</label><input id="rec_line_metrage" type="number" step="0.01" readonly></div>
        <div style="display:flex;align-items:end"><button class="success w-100" onclick="addReceptionLine()">Ajouter la ligne</button></div>
      </div>
      <div class="small muted" style="margin-top:8px">Le total mÂ² se calcule automatiquement : nb de caisse Ã— mÂ²/caisse de la rÃ©fÃ©rence sÃ©lectionnÃ©e.</div>
    </div>

    <div class="card details-card">
      <div class="toolbar"><h2 class="section-title" style="margin:0">DÃ©tails du bon d'entrÃ©e</h2><span class="pill">${fmt(totalMetrage)} mÂ²</span></div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Date</th><th>BL</th><th>Fournisseur</th><th>Transporteur</th><th>Chauffeur</th><th>Matricule</th><th>RÃ©fÃ©rence</th><th>Format</th><th>Nb de caisse</th><th>Total mÂ²</th><th></th></tr></thead>
          <tbody>
            ${d.lines.map(l => `<tr>
              <td>${esc(d.date)}</td>
              <td>${esc(d.bl||'')}</td>
      ...
      <div class="footer-actions">
        <button class="success" onclick="validateReception()">Valider le bon de rÃ©ception</button>
      </div>
    </div>`;
...
function addReceptionLine(){
  ...
  if(!ref){ alert('Choisis une rÃ©fÃ©rence.'); return; }
  if(!currentHeader.fournisseur){ alert("Choisis le fournisseur / usine avant d'ajouter une ligne."); return; }
  if(boxes <= 0){ alert('Le nombre de caisse doit Ãªtre supÃ©rieur Ã  0.'); return; }
  if(metrage <= 0){ alert('Impossible de calculer le total mÂ² pour cette rÃ©fÃ©rence. VÃ©rifie le mÂ²/caisse dans la Base.'); return; }
  const exists = state.receptionDraft.lines.some(l => String(l.referenceId||'') === String(ref.id||''));
  if(exists){ alert('Cette rÃ©fÃ©rence existe dÃ©jÃ  dans ce bon. Utilise le bouton Modifier sur la ligne existante.'); return; }
  if(state.receptionDraft.lines.length){
    ...
    if(!sameReceptionHeader(locked, currentHeader)){
      alert("Tu ne peux pas ajouter une ligne avec un autre fournisseur, transporteur, chauffeur, matricule, date ou BL dans le mÃªme bon. Valide d'abord le bon actuel.");
      return;
    }
  }else{
...
function validateReception(){
  ...
  if(d.editingArchiveId){
    ...
    if(idx === -1){ alert("Le bon d'archive Ã  modifier est introuvable."); return; }
    const original = state.receptions[idx];
    archivedDoc = {...JSON.parse(JSON.stringify(d)), id:original.id, number:original.number, sequence:original.sequence, createdAt:original.createdAt, status:'validÃ©', editingArchiveId:null, editingOriginal:null};
    state.receptions[idx] = archivedDoc;
  }else{
    const num = consumeNextNumber('BR');
    archivedDoc = {...JSON.parse(JSON.stringify(d)), id:uid(), number:num.number, sequence:num.sequence, createdAt:new Date().toISOString(), status:'validÃ©', editingArchiveId:null, editingOriginal:null};
  ...
  alert(d.editingArchiveId ? 'Bon de rÃ©ception modifiÃ©, imprimÃ© et archivÃ©.' : 'Bon de rÃ©ception validÃ©, imprimÃ© et archivÃ©.');
  renderReception();
}
...
function renderSortie(){
  ...
  document.getElementById('view').innerHTML = `
    <div class="card">
      ...
      <div class="subtitle">Saisie des marchandises sorties du dÃ©pÃ´t avec calcul automatique du mÃ©trage et validation souple.</div>
      <div class="row">
        <div><label>NÂ° bon</label><input value="${esc(d.number)}" readonly></div>
        <div><label>Date</label><input type="date" value="${esc(d.date)}" onchange="state.sortieDraft.date=this.value;saveState()"></div>
        <div><label>Type client</label><select onchange="changeClientType(this.value)"><option value="divers" ${d.clientType==='divers'?'selected':''}>Client divers</option><option value="compte" ${d.clientType==='compte'?'selected':''}>Client Ã  compte</option></select></div>
    ...
    <div class="card entry-card">
      ...
      <div class="row">
        <div>
          <label>RÃ©fÃ©rence</label>
          <div class="autocomplete">
            <input id="sort_ref_search" placeholder="Tape la rÃ©fÃ©rence" oninput="renderRefSuggestions('sort');updateSortieLineTotals()" onfocus="renderRefSuggestions('sort')" autocomplete="off">
        ...
        <div><label>mÂ² / caisse</label><input id="sort_line_m2caisse" readonly></div>
        <div><label>Total mÂ²</label><input id="sort_line_metrage" readonly></div>
    ...
    <div class="card details-card">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>RÃ©fÃ©rence</th><th>Format</th><th>Stock dispo</th><th>Nb caisse</th><th>Total mÂ²</th><th>Statut</th><th></th></tr></thead>
          <tbody>
            ${d.lines.map(l => {
...
function addSortieLine(){
  ...
  if(!ref){ alert('Choisis une rÃ©fÃ©rence.'); return; }
  if(boxes <= 0){ alert('Le nombre de caisse doit Ãªtre supÃ©rieur Ã  0.'); return; }
  if(metrage <= 0){ alert('Le total mÂ² doit Ãªtre supÃ©rieur Ã  0. VÃ©rifie la rÃ©fÃ©rence sÃ©lectionnÃ©e.'); return; }
...
function validateSortie(){
  const d = state.sortieDraft;
  if(!d.lines.length){ alert('Ajoute au moins une ligne.'); return; }
  if(d.clientType==='compte' && !d.clientCode){ alert('Choisis un client Ã  compte.'); return; }
  ...
  for(const l of d.lines){
    ...
    if(Number(l.metrage||0) > available){
      const proceed = confirm(`Stock insuffisant pour ${l.reference}. Disponible : ${fmt(available)} mÂ².

Cliquer sur OK pour valider quand mÃªme, ou Annuler pour revenir au bon.`);
      if(!proceed) return;
    }
  }
  let archived;
  if(d.editingArchiveId){
    ...
    archived = {...JSON.parse(JSON.stringify(d)), id:original.id, number:original.number, sequence:original.sequence, createdAt:original.createdAt, status:'validÃ©', editingArchiveId:null, editingOriginal:null};
    state.sorties[idx] = archived;
  }else{
    const num = consumeNextNumber('BS');
    archived = {...JSON.parse(JSON.stringify(d)), id:uid(), number:num.number, sequence:num.sequence, createdAt:new Date().toISOString(), status:'validÃ©', editingArchiveId:null, editingOriginal:null};
  ...
  alert(d.editingArchiveId ? 'Bon de sortie modifiÃ©, imprimÃ© et archivÃ©.' : 'Bon de sortie validÃ©, imprimÃ© et archivÃ©.');
  renderSortie();
}
...
function renderEtat(){
  ...
  document.getElementById('view').innerHTML = `
    <div class="card">
      <div class="toolbar"><h2 class="section-title" style="margin:0">Ã‰tat de stock du dÃ©pÃ´t</h2><button class="info" onclick="printStockState()">Imprimer Ã©tat de stock</button></div>
      <div class="row-2">
        <div>
          <label>Recherche par rÃ©fÃ©rence / format / fournisseur</label>
        ...
        <div class="stats" style="margin-top:24px">
          ...
          <div class="stat"><div class="label">RÃ©fÃ©rences actives</div><div class="value">${rows.length}</div></div>
        </div>
      </div>
    </div>
    <div class="card">
      <div class="table-wrap">
        <table>
          <thead><tr><th>RÃ©fÃ©rence</th><th>Format</th><th>Fournisseur</th><th>Stock</th><th>Nb caisse</th><th>Nb palette</th><th>Ã‰tat</th></tr></thead>
          <tbody>
            ${rows.map(r => `<tr>
              <td class="mono">${esc(r.reference)}</td><td>${esc(r.format)}</td><td>${esc(r.marque)}${getSupplierBrand(r.marque) && getSupplierBrand(r.marque)!==r.marque ? ' <span class="muted small">(' + esc(getSupplierBrand(r.marque)) + ')</span>' : ''}</td><td class="line-total">${fmt(r.stock)}</td><td>${fmt(r.stockCaisses)}</td><td>${fmt(r.stockPalettes)}</td><td>${r.stock<0?'<span class="tag bad">NÃ©gatif</span>':r.stock===0?'<span class="tag warn-tag">Ã‰puisÃ©</span>':r.stock<20?'<span class="tag warn-tag">Faible</span>':'<span class="tag ok">Correct</span>'}</td>
            </tr>`).join('') || `<tr><td colspan="7" class="muted">Aucune donnÃ©e de stock.</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>`;
...
function getSyntheseBounds(period){
  ...
  if(period === 'day'){
    ...
    return { start, end, label:`Jour sÃ©lectionnÃ© Â· ${start.toLocaleDateString('fr-FR')}` };
  }
  if(period === 'month'){
    ...
    return { start, end, label:`Mois sÃ©lectionnÃ© Â· ${start.toLocaleDateString('fr-FR', { month:'long', year:'numeric' })}` };
  }
  if(period === 'quarter'){
    ...
    return { start, end, label:`Trimestre sÃ©lectionnÃ© Â· T${q} ${y}` };
  }
  ...
  return { start, end, label:`AnnÃ©e sÃ©lectionnÃ©e Â· ${yearValue}` };
}
function syntheseSelectorHtml(period){
  ...
  return `<div><label>Choisir l'annÃ©e</label><select onchange="setSyntheseSelector('year', this.value)">${yearOptions}</select></div>`;
}
...
function printSynthese(){
  ...
  const inRows = data.receptionDetails.map(r => `<tr><td>${esc(r.date)}</td><td>${esc(r.number)}</td><td>${esc(r.reference)}</td><td>${esc(r.format)}</td><td>${esc(r.fournisseur)}</td><td>${esc(r.tiers)}</td><td>${fmt(r.caisses)}</td><td><strong>${fmt(r.metrage)} mÂ²</strong></td></tr>`).join('') || `<tr><td colspan="8" style="text-align:center;color:#6b7280">Aucune entrÃ©e sur cette pÃ©riode.</td></tr>`;
  const outRows = data.sortieDetails.map(r => `<tr><td>${esc(r.date)}</td><td>${esc(r.number)}</td><td>${esc(r.reference)}</td><td>${esc(r.format)}</td><td>${esc(r.fournisseur)}</td><td>${esc(r.tiers)}</td><td>${fmt(r.caisses)}</td><td><strong>${fmt(r.metrage)} mÂ²</strong></td></tr>`).join('') || `<tr><td colspan="8" style="text-align:center;color:#6b7280">Aucune sortie sur cette pÃ©riode.</td></tr>`;
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>SynthÃ¨se dÃ©pÃ´t</title><style>
  ...
  printInline(html, 'SynthÃ¨se dÃ©pÃ´t');
}
  ...
  target.innerHTML = `
    <div class="card">
      <div class="toolbar" style="align-items:flex-start;gap:12px;flex-wrap:wrap">
        <div>
          <h2 class="section-title">SynthÃ¨se dÃ©pÃ´t</h2>
          <div class="subtitle">Lecture par pÃ©riode : jour, mois, trimestre ou annÃ©e.</div>
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end">
          <div class="segmented">
            ...
            <button class="${data.period==='year'?'primary':'ghost'}" onclick="setSynthesePeriod('year')">AnnÃ©e</button>
          </div>
          <button class="info" onclick="printSynthese()">Imprimer la synthÃ¨se</button>
        </div>
      </div>
      <div class="row-2" style="margin-top:10px">
        ${syntheseSelectorHtml(data.period)}
        <div class="notice">Choisis d'abord le type de pÃ©riode, puis sÃ©lectionne prÃ©cisÃ©ment le jour, le mois, le trimestre ou l'annÃ©e Ã  analyser.</div>
      </div>
      <div class="notice" style="margin-top:10px">PÃ©riode analysÃ©e : <strong>${esc(data.bounds.label)}</strong></div>
      <div class="stats">
        <div class="stat"><div class="label">MÃ©trage entrÃ©</div><div class="value">${fmt(data.totalIn)}</div></div>
        <div class="stat"><div class="label">MÃ©trage sorti</div><div class="value">${fmt(data.totalOut)}</div></div>
        <div class="stat"><div class="label">Stock actuel</div><div class="value">${fmt(data.totalStock)}</div></div>
        <div class="stat"><div class="label">Bons traitÃ©s</div><div class="value">${data.nbReceptions + data.nbSorties}</div></div>
      </div>
    </div>
    <div class="split">
      <div class="card">
        <h2 class="section-title">Marchandise vendue par rÃ©fÃ©rence</h2>
        <div class="table-wrap">
          <table>
            <thead><tr><th>RÃ©fÃ©rence</th><th>Format</th><th>Fournisseur</th><th>MÃ©trage vendu</th></tr></thead>
            <tbody>
              ${data.topRefs.map(r=>`<tr><td>${esc(r.label)}</td><td>${esc(r.format)}</td><td>${esc(r.fournisseur)}</td><td class="line-total">${fmt(r.valueOut)}</td></tr>`).join('') || `<tr><td colspan="4" class="muted">Aucune sortie enregistrÃ©e sur cette pÃ©riode.</td></tr>`}
            </tbody>
          </table>
        </div>
      </div>
      <div class="card">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Format</th><th>MÃ©trage vendu</th><th>Lecture</th></tr></thead>
            <tbody>
              ${data.topFormats.map(f=>`<tr><td>${esc(f.label)}</td><td class="line-total">${fmt(f.value)}</td><td>${data.topFormats[0]?.label===f.label?'<span class="tag ok">Top rotation</span>':'<span class="tag info-tag">Ã€ suivre</span>'}</td></tr>`).join('') || `<tr><td colspan="3" class="muted">Aucune sortie enregistrÃ©e sur cette pÃ©riode.</td></tr>`}
            </tbody>
          </table>
        </div>
      </div>
    ...
    <div class="card">
      <h2 class="section-title">Lecture mÃ©tier</h2>
      <div class="notice">
        Cette zone sert Ã  contrÃ´ler le dÃ©pÃ´t selon la pÃ©riode choisie. Le mÃ©trage entrÃ© et le mÃ©trage sorti sont recalculÃ©s sur le jour, le mois, le trimestre ou lâ€™annÃ©e. Le <strong>stock actuel</strong> reste le stock thÃ©orique global du dÃ©pÃ´t Ã  lâ€™instant prÃ©sent. Le bouton d'impression sort une fiche dÃ©taillÃ©e avec les entrÃ©es et les sorties de la pÃ©riode.
      </div>
    </div>`;
}
...
function renderMouvements(mountId='view'){
  ...
  target.innerHTML = `
    <div class="card">
      <h2 class="section-title">Mouvements par rÃ©fÃ©rence</h2>
      <div class="subtitle">Utilise cette partie quand le stock systÃ¨me et le stock physique ne correspondent pas.</div>
      <div class="row-2">
        <div>
          <label>Choisir une rÃ©fÃ©rence</label>
          <div class="autocomplete">
            <input id="mov_ref_search" value="${esc(ref?.reference || '')}" placeholder="Tape la rÃ©fÃ©rence comme dans les bons" oninput="document.getElementById('mov_ref_id').value=''; renderRefSuggestions('mov')" onfocus="renderRefSuggestions('mov')">
            <input type="hidden" id="mov_ref_id" value="${esc(currentRefId)}">
            <div id="movSuggestions" class="autocomplete-list hidden"></div>
          </div>
        </div>
        <div class="notice">Le tableau reconstruit toutes les entrÃ©es et toutes les sorties dans lâ€™ordre chronologique afin dâ€™identifier lâ€™origine dâ€™un Ã©cart.</div>
      </div>
    </div>
    <div class="card">
      <div class="toolbar"><h2 class="section-title" style="margin:0">${ref ? esc(ref.reference) + ' Â· ' + esc(ref.format) : 'Aucune rÃ©fÃ©rence'}</h2>${ref ? `<button class="info" onclick="printReferenceMoves('${ref.id}')">Imprimer la fiche mouvement</button>` : ''}</div>
      <div class="table-wrap">
        <table>
          <thead><tr><th>Date</th><th>Type</th><th>NÂ° bon</th><th>Tiers</th><th>Mouvement mÂ²</th><th>Stock cumulÃ©</th></tr></thead>
          <tbody>
            ${moves.map(m => `<tr><td>${esc(m.date)}</td><td>${m.type==='ENTRÃ‰E'?'<span class="tag ok">EntrÃ©e</span>':'<span class="tag info-tag">Sortie</span>'}</td><td class="mono">${esc(m.number)}</td><td>${esc(m.tiers)}</td><td class="line-total">${m.metrage>0?'+':''}${fmt(m.metrage)}</td><td class="line-total">${fmt(m.running)}</td></tr>`).join('') || `<tr><td colspan="6" class="muted">Aucun mouvement pour cette rÃ©fÃ©rence.</td></tr>`}
          </tbody>
        </table>
      </div>
    </div>`;
...
function renderArchives(){
  ...
  const recRows = recs.map(d => {
    ...
    const badge = d.status === 'annulÃ©' ? `<span class="tag bad">ANNULÃ‰</span>` : `<span class="tag ok">VALIDÃ‰</span>`;
    const actions = d.status === 'annulÃ©'
  ...
  }).join('') || `<tr><td colspan="7" class="muted">Aucun bon rÃ©ception archivÃ©.</td></tr>`;
  ...
  }).join('') || `<tr><td colspan="6" class="muted">Aucun bon sortie archivÃ©.</td></tr>`;
  document.getElementById('view').innerHTML = `
    <div class="stack" style="display:grid;gap:16px">
      <details class="card" open>
        <summary class="toolbar" style="cursor:pointer;list-style:none;user-select:none">
          <div style="display:flex;align-items:center;gap:10px">
            ...
            <h2 class="section-title" style="margin:0">Archives des rÃ©ceptions</h2>
          </div>
          <span class="pill">${recs.length}</span>
        </summary>
        <div class="table-wrap" style="margin-top:12px">
          <table>
            <thead><tr><th>NÂ° entrÃ©e</th><th>Date</th><th>BL</th><th>Fournisseur</th><th>Statut</th><th>Lignes</th><th></th></tr></thead>
            <tbody>${recRows}</tbody>
          </table>
        </div>
      </details>
      <details class="card" open>
        ...
        <div class="table-wrap" style="margin-top:12px">
          <table>
            <thead><tr><th>NÂ° bon</th><th>Date</th><th>Client</th><th>Statut</th><th>Lignes</th><th></th></tr></thead>
            <tbody>${outRows}</tbody>
          </table>
        </div>
      </details>
...
function buildReceptionDoc(doc){
  const rows = doc.lines.map((l)=>`<tr><td>${esc(l.reference)}</td><td>${Number(l.boxes||0)||''}</td><td>${fmt(l.metrage)} mÂ²</td></tr>`).join('');
  ...
  <body onload="window.print();setTimeout(()=>window.close(),200);">
    <div class="doc-page">
      ...
      <div class="panel">
        <div class="doc-header">
          ...
          <div class="doc-header-right">
            <div>Bon d'entrÃ©e</div>
            <div>NÂ° entrÃ©e : ${esc(doc.number)}</div>
      ...
      <div class="panel">
        <h2 class="section-title">BON DE RÃ‰CEPTION</h2>
        <div class="top-info">
        ...
        <div class="table-shell">
          <table>
            ...
            <thead><tr><th>RÃ©fÃ©rence</th><th>Nb de caisse</th><th>Total mÂ²</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
        <div class="totals"><div class="box"><div>Total mÃ©trage entrÃ©e</div><div>${total} mÂ²</div></div></div>
        <div class="signature-grid"><div class="signature-box">Visa rÃ©ception</div><div class="signature-box">Visa transport</div><div class="signature-box">Visa magasin</div><div class="signature-box">Visa direction</div></div>
      </div>
    </div>
}
...
function buildSortieDoc(doc){
  const rows = doc.lines.map((l)=>`<tr><td>${esc(l.reference)}</td><td>${Number(l.boxes||0)||''}</td><td>${fmt(l.metrage)} mÂ²</td></tr>`).join('');
  const total = fmt(doc.lines.reduce((s,l)=>s+Number(l.metrage||0),0));
  const clientTypeLabel = doc.clientType === 'compte' ? 'Client Ã  compte' : 'Client divers';
  ...
  <body onload="window.print();setTimeout(()=>window.close(),200);">
    <div class="doc-page">
      ...
      <div class="panel">
        ...
        <div class="totals"><div class="box"><div>Total mÃ©trage sortie</div><div>${total} mÂ²</div></div></div>
...
function openPrint(html){ const win = window.open('','_blank'); if(!win){ alert("Le navigateur a bloquÃ© l'ouverture de la fenÃªtre d'impression."); return; } win.document.open(); win.document.write(html); win.document.close(); win.focus(); setTimeout(()=>win.print(),300); }
...
function printReceptionArchive(id){ const doc = state.receptions.find(d=>d.id===id); if(!doc) return; if(doc.status === 'annulÃ©'){ alert("Un bon de rÃ©ception annulÃ© ne peut plus Ãªtre imprimÃ©."); return; } openPrint(buildReceptionDoc(doc)); }
function cancelReceptionArchive(id){
  const doc = state.receptions.find(d=>d.id===id);
  if(!doc) return;
  if(doc.status === 'annulÃ©'){ alert('Ce bon de rÃ©ception est dÃ©jÃ  annulÃ©.'); return; }
  if(!confirm(`Annuler ${doc.number} ? Les quantitÃ©s de ce bon seront retirÃ©es du stock.`)) return;
  doc.status = 'annulÃ©';
...
function deleteReceptionArchive(id){
  const doc = state.receptions.find(d=>d.id===id);
  if(!doc) return;
  if(!confirm(`Supprimer dÃ©finitivement ${doc.number} de l'archive ? Cette action est rÃ©servÃ©e aux essais. Son numÃ©ro redeviendra rÃ©utilisable.`)) return;
...
function printSortieArchive(id){ const doc = state.sorties.find(d=>d.id===id); if(!doc) return; if(doc.status === 'annulÃ©'){ alert("Un bon de sortie annulÃ© ne peut plus Ãªtre imprimÃ©."); return; } openPrint(buildSortieDoc(doc)); }
function cancelSortieArchive(id){
  const doc = state.sorties.find(d=>d.id===id);
  if(!doc) return;
  if(doc.status === 'annulÃ©'){ alert('Ce bon de sortie est dÃ©jÃ  annulÃ©.'); return; }
  if(!confirm(`Annuler ${doc.number} ? Les quantitÃ©s de ce bon seront remises au stock.`)) return;
  doc.status = 'annulÃ©';
  doc.cancelledAt = new Date().toISOString();
  saveState();
  renderArchives();
...
function printStockState(){
  ...
  const rows = stockRows.map(r=>`<tr><td>${esc(r.reference)}</td><td>${esc(r.marque)}</td><td>${esc(r.format)}</td><td><strong>${fmt(r.stock)} mÂ²</strong></td><td>${fmt(r.stockCaisses)}</td><td>${fmt(r.stockPalettes)}</td></tr>`).join('');
  const logoSrc = document.querySelector('.oramed-logo-img')?.src || '';
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Ã‰tat de stock</title><style>
    *{box-sizing:border-box}
    body{font-family:Arial,sans-serif;color:#111827;background:#fff;padding:18px}
  ...
  printInline(html, 'Ã‰tat de stock');
}
function printReferenceMoves(id){
  const ref = getReferenceById(id); if(!ref) return;
  const rows = computeMovements(id).map(m=>`<tr><td>${esc(m.date)}</td><td>${m.type==='ENTRÃ‰E'?'EntrÃ©e':'Sortie'}</td><td>${esc(m.number)}</td><td>${esc(m.tiers)}</td><td>${m.metrage>0?'+':''}${fmt(m.metrage)} mÂ²</td><td>${fmt(m.running)} mÂ²</td></tr>`).join('') || `<tr><td colspan="6" style="text-align:center;color:#6b7280">Aucun mouvement.</td></tr>`;
  ...
  printInline(html, 'Fiche mouvement par rÃ©fÃ©rence');
}
function exportData(){
...
function importDataPrompt(){
  ...
  input.onchange = e => {
    ...
    const reader = new FileReader(); reader.onload = () => {
      try{ state = Object.assign(defaultState(), JSON.parse(reader.result)); ensureDraftNumbers(); saveState(); renderTabs(); syncRoleBadge(); renderActiveTab(); alert('Import terminÃ©.'); }
